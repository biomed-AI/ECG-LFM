...
from fairseq_signals.modules.linear import Linear
...
class MOELayer(Base):
    ...
    def forward(
        ...
        if isinstance(expert_outputs[0], tuple):
            # When expert module (FeedForwardModule in Conformer for example) has auxiliary fields
            expert_outputs = [eo[0] for eo in expert_outputs]
