...
class ConformerEncoderLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            embed_dim: Input embedding dimension 1024
            ffn_embed_dim: FFN layer dimension 4096
            attention_heads: Number of attention heads in MHA 16
            dropout: dropout value
...
class BranchformerEncoderLayer(torch.nn.Module):
    ...
    def forward(
        ...
        #########
        residual = x
        x_attn = self.self_attn_layer_norm(x)
        if self.pos_enc_type == "rel_pos":
        ...
        x_attn = self.self_attn_dropout(x_attn)
        #x_conv = self.self_attn_layer_norm(x)
        x_conv = x.transpose(0, 1)
        x_conv = self.conv_module(x_conv)
        x_conv = x_conv.transpose(0, 1)
        combined = torch.cat([x_attn, x_conv], dim=-1)
