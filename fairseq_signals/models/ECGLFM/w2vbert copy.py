...
from fairseq_signals import utils1
from fairseq_signals.data.data_utils import compute_mask_indices
from fairseq_signals.dataclass import ChoiceEnum, FairseqDataclass
from fairseq_signals.distributed import fsdp_wrap
from fairseq_signals.models import register_model
#from fairseq_signals.models.fairseq_model import BaseFairseqModel
...
class ECGLFMConfig(ECGTransformerConfig):
    ...
    w2v2_encoder_layers: int = field(
        default=6, metadata={"help": "num w2v2 encoder layers in the transformer"}
    )
    encoder_layers: int = field(
        default=12,
        metadata={
            "help": "num encoder layers (including w2v2 and mlm) in the transformer"
        },
    )
    ...
    activation_fn: ChoiceEnum(utils1.get_available_activation_fns()) = field(
    ...
    mlm_dropout: float = field(
        default=0.1, metadata={"help": "dropout probability for the mlm conformer"}
    )
    ...
    is_moe_ffn2: bool = field(
        default=False, metadata={"help": "Whether ffn2 in conformer is MoE"}
    )
...
class ECGLFMModel(ECGTransformerModel):
    def __init__(self, cfg: ECGLFMConfig):
        ...
        # Used to extract w2v2 features from intermediate layers
        self.w2v2_encoder_layers = cfg.w2v2_encoder_layers
        self.quantizer = None
        ...
        # NOTE: mlm_proj has to be updated jointly with mlm_targets size below
        self.mlm_proj = nn.Linear(cfg.encoder_embed_dim, cfg.latent_vars)
    ...
    def sample_negatives(self, y, num, padding_count=None):
        ...
        y = y.view(-1, fsz)  # BTC => (BxT)C 742, 768
        ...
        with torch.no_grad():
            ...
            if self.n_negatives > 0: #100
                tszs = (
                    buffered_arange(num)
                    .unsqueeze(-1)
                    .expand(-1, self.n_negatives)
                    .flatten()
                ) #num * self.n_negatives
            ...
            if self.cross_sample_negatives > 0: #0
                tszs = (
                    buffered_arange(num)
                    .unsqueeze(-1)
        ...
        negs = y[neg_idxs.view(-1)] #y 742, 768 neg_idxs 2, 37100 negs 74200, 768

        negs = negs.view(
            bsz, num, self.n_negatives + self.cross_sample_negatives, fsz
        ).permute(
            2, 0, 1, 3
        )  # to NxBxTxC 100, 2, 371, 768
        return negs, neg_idxs

    def compute_w2v2_preds(self, x, y, negatives):
        neg_is_pos = (y == negatives).all(-1) #100, 2, 371
    ...
    def compute_mlm_preds(self, x):
        logits = self.mlm_proj(x)
        return logits
    ...
    def forward(
        ...
        if self.cfg.fbank_features <= 0:
            ...
            input_lengths = torch.tensor([source[0].size()[0]] * len(source)) #tensor([1583, 1583])
            if padding_mask is not None and padding_mask.any(): #None
                input_lengths = (1 - padding_mask.long()).sum(-1)
            features = self.sample_fbank(source.permute(1, 0, 2), input_lengths) 
            features = features.permute(1, 0, 2)  # B x T x D 2, 791, 160

        features_pen = features.float().pow(2).mean()
        features = self.layer_norm(features)
        unmasked_features = features.clone()

        if padding_mask is not None and padding_mask.any(): #False
            input_lengths = (1 - padding_mask.long()).sum(-1)
        ...
        time_steps_to_drop = features.size(1) % self.crop_seq_to_multiple #0
        ...
        features = self.dropout_input(features) #8, 156, 768
        unmasked_features = self.dropout_features(unmasked_features) #8, 156, 256
        num_vars = None
        code_ppl = None
        prob_ppl = None
        curr_temp = None
        ...
        x, layer_results = self.encoder(
            x, padding_mask=padding_mask, layer=self.cfg.encoder_layers - 1
        )#x: 8, 156, 768
        ...
        x_w2v2 = layer_results[self.w2v2_encoder_layers - 1][0] # self.w2v2_encoder_layers: 6

        # TxBxC -> BxTxC
        x_w2v2 = x_w2v2.transpose(0, 1)    #2, 791, 1024     
        
        if self.negatives_from_everywhere: #False
        ...
        if self.codebook_negatives > 0: #0
        ...
        x_w2v2 = x_w2v2[mask_indices].view(x_w2v2.size(0), -1, x_w2v2.size(-1)) #4, 156, 768 > 4, 62, 768
        x_w2v2 = self.final_proj(x_w2v2)  # encoder_embed_dim -> final_dim 4, 62, 1024 > 4, 62, 768

        x_w2v2 = self.compute_w2v2_preds(x_w2v2, y, negs) #4, 62, 768 > 101, 4, 62

        x = self.compute_mlm_preds(x)  # encoder_embed_dim -> num_classes 4, 62, 1024 > 4, 62, 1024
        # NOTE: Currently only taking the first index from the first var group as mlm targets.
        mlm_targets = q["targets"][:, :, 0] #4, 62, contain 1024 classes
    ...
    def get_w2v2_logits(self, net_output):
        logits = net_output["x_w2v2"]
        logits = logits.transpose(0, 2)
        logits = logits.reshape(-1, logits.size(-1))
        return logits

    def get_mlm_targets(self, sample, net_output, expand_steps=True):
        x = net_output["mlm_targets"]
        x = x.view(-1)
        return x.long()

    def get_w2v2_targets(self, sample, net_output, expand_steps=True):
        x = net_output["x_w2v2"]
...
class ConformerEncoder(TransformerEncoder):
    def build_encoder_layer_ECGLFM(
        ...
        layer = ConformerWav2Vec2EncoderLayer(
            ...
            attention_heads=args.encoder_attention_heads,
            dropout=args.dropout
            if layer_count < args.w2v2_encoder_layers
            else args.mlm_dropout,
            depthwise_conv_kernel_size=args.depthwise_conv_kernel_size,
            activation_fn="swish",
            attn_type=args.attn_type,
            pos_enc_type=args.pos_enc_type,
            use_fp16=args.fp16,  # only used for rope
            is_moe_layer=(args.moe_expert_count > 1)
            and (layer_count % args.moe_freq == 0),
            is_moe_ffn2=args.is_moe_ffn2,
        )
        layer = fsdp_wrap(layer)
        if args.checkpoint_activations:
            layer = checkpoint_wrapper(layer)
    ...
    def extract_features(self, x, padding_mask=None, tgt_layer=None):
        ...
        for i, layer in enumerate(self.layers):
            dropout_probability = (
                np.random.random()
            )  # NOTE: though chance is small, what if dropout_probability is sampled as 0?
            if not self.training or (dropout_probability > self.layerdrop):
                x, z = layer(
                    x,
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # Initialize blocks
        self.activation_fn = utils1.get_activation_fn(activation_fn)
