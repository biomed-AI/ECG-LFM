...
class Wav2Vec2Model(ECGTransformerModel):
    ...
    def forward(
        ...
        print(source.shape) #32, 12, 2500
        if self.feature_grad_mult > 0: #0.1
            features = self.feature_extractor(source) #32, 256, 156
        ...
        features = self.dropout_input(features) #4, 156, 768
        unmasked_features = self.dropout_features(unmasked_features) #32, 156, 256
        ...
        if mask:
            ...
            ) #x: 32, 156, 768; mask_indices: 32, 156
        ...
        x = x + x_conv #32, 156, 768

        x = self.encoder(x, padding_mask = padding_mask) #32, 156, 768
        ...
        x = self.final_proj(x) #32, 59, 768 > 32, 59, 256
        x = self.compute_preds(x, y, negs) #101, 32, 62
