...
@dataclass
class ECGLFMCriterionConfig(Dataclass):
    infonce: bool = field(
        default=False,
        metadata={
...
@dataclass
class ECGLFMWithCMSCCriterionConfig(ECGLFMCriterionConfig, CMSCCriterionConfig):
    cmsc_weights: Optional[float] = field(
        default=None,
        metadata={"help": "weights for cmsc loss terms"}
    )

@register_criterion("ECGLFM_with_cmsc", dataclass=ECGLFMWithCMSCCriterionConfig)
class ECGLFMCriterion(BaseCriterion):
    # def __init__(
    #     self,
    #     task,
    #     infonce=False,
    ...
    def __init__(self, cfg: ECGLFMCriterionConfig, task: Task):
        super().__init__(task)

        self.infonce = cfg.infonce
        self.w2v2_loss_weight = cfg.w2v2_loss_weight
        self.mlm_loss_weight = cfg.mlm_loss_weight
        self.mlm_label_smoothing = cfg.mlm_label_smoothing
    ...
    def forward(self, model, sample, reduce=True, save_outputs=False):
        ...
        if save_outputs:
            self.store(w2v2_logits, w2v2_targets.float())
