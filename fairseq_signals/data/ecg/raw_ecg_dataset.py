...
class RawECGDataset(BaseDataset):
    ...
    def collator(self, samples):
        ...
        if self.label:
            # out["timey"] = torch.stack([s["timey"] for s in samples])
            # out["death"] = torch.stack([s["death"] for s in samples])
            # out["age"] = torch.stack([s["age"] for s in samples])
    ...
    def __getitem__(self, index):
        ...
        if self.label:
            if self.label_array is not None:
                ...
                # res["timey"] = torch.from_numpy(ecg['timey'].squeeze())
                # res["death"] = torch.from_numpy(ecg['death'].squeeze())
                # res["age"] = torch.from_numpy(ecg['age'].squeeze())
