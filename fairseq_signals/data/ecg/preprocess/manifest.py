...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        metavar="D",
        help="percentage of data to use as validation set (between 0 and 0.5)",
    )
    parser.add_argument(
        "--test-percent",
        default=0.1,
        type=float,
        metavar="D",
        help="percentage of data to use as test set (between 0 and 0.5)",
    )
    parser.add_argument(
        "--group-by",
        #default='_',
...
def main(args):
    ...
    with open(os.path.join(args.dest, "train.tsv"), "w") as train_f, open(
        ...
        fnames = []
        print(args.root)
        for fname in tqdm(os.listdir(args.root)):
            fnames.append(os.path.join(args.root, fname))
        ...
        if groupby is not None:
            ...
            test_len = int(len(groups) * args.test_percent)
            ...
            test_len = int(len(fnames) * args.test_percent)
